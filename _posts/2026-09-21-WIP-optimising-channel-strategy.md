...
We confirmed a single GA4 property spanning both the app (via Firebase) and the website (via gtag), registered the custom dimensions listed in the Data Overview above, and validated event delivery in DebugView before letting any baseline window run.

```
# TODO: insert the GA4 / Firebase configuration snippet
# used to register custom dimensions and confirm the
# single-property setup across app and web data streams
```
